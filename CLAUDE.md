...
> The backend is fully wired (v1.0.0). **Microsoft login is real** — `MainWindowViewModel` delegates to `Services/AuthService` (CmlLib `JELoginHandler` via `JELoginHandlerBuilder.BuildDefault()`): interactive system-browser auth, silent re-login from cache on startup, and real sign-out; the `MSession` is held in the shell. **Launch is real** — `HomePageViewModel.PlayAsync` uses `GameLauncher` (CmlLib + NeoForge installer) and `ModInstaller` to download/verify files (placed by `target` into `mods/`/`resourcepacks/`/`shaderpacks/`), applies the modpack's `overrides` bundle via `OverridesInstaller`, then starts the game. Official modpacks, news and launcher releases come from the **TCMine-Server** project (ASP.NET minimal API + SQLite/EF Core + a Blazor admin at `/admin`); the launcher auto-updates via Velopack. The solution also has **TCMine-IconGen** (generates the app icon).
